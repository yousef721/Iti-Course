...
private:
    ...
    Circle *pCircle;   // Aggregation: Picture contain Circles but does not own them
    Rect *pRectangle;  // Aggregation: Picture contain Rectangles but does not own them
    Line *pLine;       // Aggregation: Picture contain Lines but does not own them
public:
    ...
    // Association: Picture "uses" these objects by calling Draw()
    void Paint()
    {
        int i;
        // Circles
